              ...
              <div className="w-full md:w-1/2 p-6 flex flex-col ">
                <div className="flex items-center justify-between mb-4 ">
                  {edit ? (
                    ...
                    <h1 className="text-2xl font-bold">{pin.pinTitle}</h1>
                  )}

                  {pin.pinCreatedUser && pin.pinCreatedUser._id === user._id && (
                ...
                {pin.pinCreatedUser && (
                  <div className="flex items-center justify-between border-b pb-4 mb-4">
                    <div className="flex items-center">
                      <Link to={`/user/${pin.pinCreatedUser._id}`}>
                        <div className="rounded-full h-12 w-12 bg-gray-300 flex items-center justify-center">
                          <span className="font-bold">
                            {pin.pinCreatedUser.name.slice(0, 1)}
                          </span>
                        </div>
                      </Link>
                      <div className="ml-4">
                        <h2 className="text-lg font-semibold">
                          {pin.pinCreatedUser.name}
                        </h2>
                        <p className="text-gray-500">
                          {pin.pinCreatedUser.followers.length} Followers
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex items-center mt-4">
                  <div className="rounded-full h-12 w-12 bg-gray-300 flex items-center justify-center mr-4">
                    <span className="font-bold">
                      {pin.pinCreatedUser && pin.pinCreatedUser.name.slice(0, 1)}
                    </span>
                  </div>
                ...
                <div className="overflow-y-auto h-64">
                  {pin.pinComments && pin.pinComments.length > 0 ? (
                    pin.pinComments.map((e, i) => (
                      <div className="flex items-center justify-between mb-4">
